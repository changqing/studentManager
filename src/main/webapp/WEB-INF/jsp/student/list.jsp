...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    <div id="mainContent">
		<div id="vLocation">
			当前位置：账号管理 / <a href="javascript:void(0)" style="color: #0088cc;">账号列表</a>
		</div>
		...
		<table id="content" class="t" cellpadding="0" cellspacing="0">
				<c:if test="${list.size() == 0 }">
					<tr><td style="color: red;font-weight: bold;width: 100%;">未查询到学生信息</td></tr>
				</c:if>
				<c:if test="${list.size() > 0 }">
					<c:forEach items="${list }" var="s" varStatus="st">
						<tr>
							<td width="15%">${st.index + 1 }</td>
							<td width="30%" style="color: #f66;font-weight: bold;">${s.username }</td>
							<td width="20%">${s.truename }</td>
							<td width="10%">${s.age }</td>
							<td width="10%">${s.sex }</td>
							<td width="15%">
								<a href="${pageContext.request.contextPath}/student_delete?id=${s.id }" title="删除学生信息" style="margin-left: 20px;">
