...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    <div id="mainContent">
		<div id="vLocation">
			当前位置：课程管理 / <a href="javascript:void(0)" style="color: #0088cc;">我的课表</a>
		</div>
				...
				<c:if test="${list.size() > 0 }">
					<c:forEach items="${list }" var="c" varStatus="st">
						<tr>
							...
							<td width="20%" style="color: #f66;font-weight: bold;">${c.lname }</td>
							<td width="10%">${c.tname }</td>
							<td width="15%">${c.address }</td>
							<td width="15%">${c.time }</td>
							<td width="10%" style="color: #f66;font-weight: bold;">${c.score }</td>
							<td width="20%">
								<a href="${pageContext.request.contextPath}/lesson/cancel?id=${c.id }" title="取消课程" style="margin-left: 20px;">
